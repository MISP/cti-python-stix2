...
def test_making_new_version():
    campaign_v1 = misp_lib_stix2.v20.Campaign(**CAMPAIGN_MORE_KWARGS)
...
def test_making_new_version_with_embedded_object():
    campaign_v1 = misp_lib_stix2.v20.Campaign(
        external_references=[{
            "source_name": "capec",
...
def test_versioning_error_bad_modified_value():
    ...
    assert excinfo.value.cls == misp_lib_stix2.v20.Campaign
    ...
    msg = msg.format(
        misp_lib_stix2.v20.Campaign.__name__, "modified",
...
def test_versioning_error_usetting_required_property():
    ...
    msg = msg.format(misp_lib_stix2.v20.Campaign.__name__, "name")
    assert str(excinfo.value) == msg
...
def test_revoke_unversionable():
    sco = misp_lib_stix2.v20.File(name="data.txt")
...
def test_remove_custom_stix_property():
    mal = misp_lib_stix2.v20.Malware(
        name="ColePowers",
        labels=["rootkit"],
        x_custom="armada",
        allow_custom=True,
    ...
    assert (
        misp_lib_stix2.utils.parse_into_datetime(mal["modified"], precision="millisecond") <
        misp_lib_stix2.utils.parse_into_datetime(mal_nc["modified"], precision="millisecond")
    )
...
def test_version_sco_with_modified():
    ...
    with pytest.raises(misp_lib_stix2.exceptions.TypeNotVersionableError):
        misp_lib_stix2.versioning.new_version(file_sco, name="newname.txt")

    with pytest.raises(misp_lib_stix2.exceptions.TypeNotVersionableError):
        misp_lib_stix2.versioning.revoke(file_sco)

    file_sco_obj = misp_lib_stix2.v20.File(
        name="data.txt",
        created="1973-11-23T02:31:37Z",
        modified="1991-05-13T19:24:57Z",
    )
    ...
    with pytest.raises(misp_lib_stix2.exceptions.TypeNotVersionableError):
        misp_lib_stix2.versioning.new_version(file_sco_obj, name="newname.txt")

    with pytest.raises(misp_lib_stix2.exceptions.TypeNotVersionableError):
        misp_lib_stix2.versioning.revoke(file_sco_obj)
...
def test_version_marking():
    m = misp_lib_stix2.v20.MarkingDefinition(
...
def test_version_propagate_custom():
    ...
    m_custom = misp_lib_stix2.v20.Malware(
