...
def test_location_with_some_required_properties():
    ...
    location = misp_lib_stix2.v21.Location(
        id=LOCATION_ID,
        created=now,
        modified=now,
        latitude=48.8566,
...
def test_parse_location(data):
    location = misp_lib_stix2.parse(data, version="2.1")
...
def test_location_complex_presence_constraint():
    with pytest.raises(misp_lib_stix2.exceptions.PropertyPresenceError):
...
def test_google_map_url_long_lat_provided():
    ...
    loc = misp_lib_stix2.v21.Location(
        latitude=41.862401,
        longitude=-87.616001,
    )
