...
def test_infrastructure_with_all_required_properties():
    ...
    infra = misp_lib_stix2.v21.Infrastructure(
        type="infrastructure",
...
def test_infrastructure_type_must_be_infrastructure():
    with pytest.raises(misp_lib_stix2.exceptions.InvalidValueError) as excinfo:
        misp_lib_stix2.v21.Infrastructure(type='xxx', **INFRASTRUCTURE_KWARGS)

    assert excinfo.value.cls == misp_lib_stix2.v21.Infrastructure
...
def test_infrastructure_id_must_start_with_infrastructure():
    with pytest.raises(misp_lib_stix2.exceptions.InvalidValueError) as excinfo:
        misp_lib_stix2.v21.Infrastructure(id='my-prefix--', **INFRASTRUCTURE_KWARGS)
...
def test_infrastructure_required_properties():
    with pytest.raises(misp_lib_stix2.exceptions.MissingPropertiesError) as excinfo:
        misp_lib_stix2.v21.Infrastructure()
...
def test_infrastructure_required_property_name():
    with pytest.raises(misp_lib_stix2.exceptions.MissingPropertiesError) as excinfo:
        misp_lib_stix2.v21.Infrastructure(infrastructure_types=['command-and-control'])
...
def test_invalid_kwarg_to_infrastructure():
    with pytest.raises(misp_lib_stix2.exceptions.ExtraPropertiesError) as excinfo:
        misp_lib_stix2.v21.Infrastructure(my_custom_property="foo", **INFRASTRUCTURE_KWARGS)
...
def test_parse_infrastructure(data):
    infra = misp_lib_stix2.parse(data)
...
def test_parse_infrastructure_kill_chain_phases():
    ...
    infra = misp_lib_stix2.parse(data, version="2.1")
...
def test_infrastructure_invalid_last_before_first():
    with pytest.raises(ValueError) as excinfo:
        misp_lib_stix2.v21.Infrastructure(first_seen="2017-01-01T12:34:56.000Z", last_seen="2017-01-01T12:33:56.000Z", **INFRASTRUCTURE_KWARGS)
