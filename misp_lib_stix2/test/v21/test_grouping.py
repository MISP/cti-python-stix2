...
def test_grouping_type_must_be_grouping():
    with pytest.raises(misp_lib_stix2.exceptions.InvalidValueError) as excinfo:
        misp_lib_stix2.v21.Grouping(type='xxx', **GROUPING_KWARGS)

    assert excinfo.value.cls == misp_lib_stix2.v21.Grouping
...
def test_grouping_id_must_start_with_grouping():
    with pytest.raises(misp_lib_stix2.exceptions.InvalidValueError) as excinfo:
        misp_lib_stix2.v21.Grouping(id='my-prefix--', **GROUPING_KWARGS)
...
def test_invalid_kwarg_to_grouping():
    with pytest.raises(misp_lib_stix2.exceptions.ExtraPropertiesError) as excinfo:
        misp_lib_stix2.v21.Grouping(my_custom_property="foo", **GROUPING_KWARGS)
