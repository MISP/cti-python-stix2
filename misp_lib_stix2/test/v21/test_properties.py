...
from misp_lib_stix2.v21.common import MarkingProperty
...
def test_embedded_property():
    emb_prop = EmbeddedObjectProperty(type=misp_lib_stix2.v21.EmailMIMEComponent)
    mime = misp_lib_stix2.v21.EmailMIMEComponent(
...
def test_embedded_property_dict():
    ...
    result = emb_prop.clean(mime, False)
    assert isinstance(result[0], misp_lib_stix2.v21.EmailMIMEComponent)
...
def test_extension_property_valid():
    ...
    assert isinstance(
        result[0]["windows-pebinary-ext"], misp_lib_stix2.v21.WindowsPEBinaryExt,
    )
    assert not result[1]
...
def test_observable_property_obj():
    ...
    obs = misp_lib_stix2.v21.File(name="data.dat")
    obs_dict = {
        "0": obs,
    }
...
def test_observable_property_dict():
    ...
    result = prop.clean(obs_dict, False)
    assert isinstance(result[0]["0"], misp_lib_stix2.v21.File)
...
def test_observable_property_obj_custom():
    ...
    obs = misp_lib_stix2.v21.File(name="data.dat", foo=True, allow_custom=True)
    obs_dict = {
        "0": obs,
    }
...
def test_stix_object_property_custom_prop():
    ...
    assert isinstance(result[0], misp_lib_stix2.v21.Identity)
