...
def test_report_example():
    report = misp_lib_stix2.v21.Report(
        id=REPORT_ID,
...
def test_report_example_objects_in_object_refs():
    report = misp_lib_stix2.v21.Report(
        ...
        report_types=["campaign"],
        object_refs=[
            misp_lib_stix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS),
            CAMPAIGN_ID,
            RELATIONSHIP_ID,
        ],
    )
...
def test_report_example_objects_in_object_refs_with_bad_id():
    ...
    assert excinfo.value.cls == misp_lib_stix2.v21.Report
...
def test_parse_report(data):
    rept = misp_lib_stix2.parse(data, version="2.1")
