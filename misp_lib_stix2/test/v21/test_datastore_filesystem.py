...
def test_filesystem_source_sco(fs_source):
    results = fs_source.query([misp_lib_stix2.Filter("type", "=", "directory")])
...
def test_filesystem_sink_add_python_stix_object(fs_sink, fs_source):
    ...
    camp1 = misp_lib_stix2.v21.Campaign(
        name="Hannibal",
...
def test_filesystem_sink_add_objects_list(fs_sink, fs_source):
    # add list of objects
    camp6 = misp_lib_stix2.v21.Campaign(
        name="Comanche",
...
def test_filesystem_sink_marking(fs_sink):
    marking = misp_lib_stix2.v21.MarkingDefinition(
        id="marking-definition--34098fce-860f-48ae-8e50-ebd3cc5e41da",
        created="2017-01-20T00:00:00.000Z",
        definition_type="tlp",
        definition=misp_lib_stix2.v21.TLPMarking(tlp="green"),
    )
...
def test_filesystem_store_query(fs_store):
    # query()
    tools = fs_store.query([misp_lib_stix2.Filter("tool_types", "in", "tool")])
    assert len(tools) == 2
...
def test_filesystem_store_query_single_filter(fs_store):
    query = misp_lib_stix2.Filter("tool_types", "in", "tool")
...
def test_filesystem_store_query_multiple_filters(fs_store):
    fs_store.source.filters.add(misp_lib_stix2.Filter("tool_types", "in", "tool"))
...
def test_filesystem_object_with_custom_property(fs_store):
    camp = misp_lib_stix2.v21.Campaign(
...
def test_filesystem_object_with_custom_property_in_bundle(fs_store):
    ...
    bundle = misp_lib_stix2.v21.Bundle(camp, allow_custom=True)
    fs_store.add(bundle)
